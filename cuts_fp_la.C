...
void cuts_fp_la(int nlist, int block){
...
*/
	   ...
	   if(mcptbin == ptbin){
		...
		hmTwoTracknHits->Fill(dst.dau1nhits[i],dst.dau2nhits[i],wgtrc);
                hmTrack1[centbin][ptbin]->Fill(dst.dau1nhits[i],wgtrc);
                hmTrack2[centbin][ptbin]->Fill(dst.dau2nhits[i],wgtrc);
		if(dst.dau1nhits[i]<=20||dst.dau2nhits[i]<=20) nCount += wgtrc;
		totCount += wgtrc;
           }
	}
   }

   ofstream oweight("weight_lambda_fp.txt");
...
Double_t getWeight(Int_t cent, Float_t pt){
   ...
   static TF1 levy("levy","x*(([0]*([1]-1)*([1]-2))/(2.0*3.141592654*[1]*[2]*([1]*[2]+1.115683*([1]-2))))/(1+((TMath::Sqrt((x*x)+(1.115683*1.115683))-1.115683)/([1]*[2])))^[1]",0.,8.);
ifstream ipara("Levy_parameter_Lambda.txt");
